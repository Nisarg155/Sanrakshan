                ...
                return (
                    <Badge
                        className={`rounded-md ${
                            score > 75 ? "bg-green-500 text-white" : score < 50 ? "bg-red-500 text-white" : ""
                        }`}
                        variant={score < 50 ? "destructive" : "secondary"} // Use only allowed variants
                    >
                            ...
                            <AlertDialog>
                                <AlertDialogTrigger asChild>
                                    <DropdownMenuItem onSelect={(e) =>{
                                        e.preventDefault();
                                        deletePolicy(contract._id).then(() => {})
                                    }}>
...
async function fetchPolicies(): Promise<ContractAnalysis[]> {
...
async function deletePolicy(id: string): Promise<void> {
    try {
        const response = await api.delete(`/contracts/delete/${id}`);
